...
# Set up caching with the new Streamlit caching functions
@st.cache_data
...
def fetch_sentiment(symbol):
    try:
        api_key = os.getenv("newsapi_key")  # Ensure the API key is set in the environment
        if not api_key:
            raise ValueError("API key for NewsAPI is missing!")
        ...
        if not articles:
            return 0  # Default to neutral if no articles found
        
        sentiment_score = sum(TextBlob(article['title']).sentiment.polarity for article in articles) / len(articles)
        return sentiment_score
    except Exception as e:
        st.error(f"Error in fetching sentiment: {e}")
...
if symbol:
    # Fetch stock data and sentiment score
    ...
    # Plot stock data
    fig = go.Figure()
    fig.add_trace(go.Candlestick(x=stock_data.index, open=stock_data['Open'], high=stock_data['High'], low=stock_data['Low'], close=stock_data['Close'], name='Market Data'))
    ...
    # Display numerical values of indicators over the timeframe
    st.write(f"### Numerical Values of Indicators Over Time:")
    st.write(stock_data[['RSI', 'MACD', 'SMA_20', 'SMA_50']].tail())  # Show the last few rows of indicators
    
    # Option to download data
    csv = stock_data.to_csv(index=True)
    st.download_button("Download Stock Data", csv, file_name=f"{symbol}_stock_data.csv", mime="text/csv")
