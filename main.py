...
def fetch_stock_data(symbol):
    ...
    data = stock.history(period="90d")  # 3 months of data
...
# Train Model with GridSearchCV including earnings data
@st.cache_resource
def train_model(data, symbol):
    # Fetch earnings data
    eps, revenue = fetch_earnings_data(symbol)
    
    # Adding earnings data as new features
    data['EPS'] = eps
    data['Revenue'] = revenue
    
    # Price change and target column
...
def generate_recommendation(data, sentiment_score, model, symbol):
    ...
    latest_features = np.array([[latest_data['Close'], latest_data['RSI'], latest_data['MACD'], 
                                 ...
                                 latest_data['SMA_20'], latest_data['SMA_50'], eps, revenue]])
    
    # Predict option
    prediction_prob = model.predict_proba(latest_features)[0][1]
    option = "Call" if prediction_prob > 0.5 else "Put"
    
    # Adjust the option based on sentiment
    if sentiment_score > 0.2 and option == "Put":
        option = "Call"
    elif sentiment_score < -0.2 and option == "Call":
        option = "Put"
    
    # Determine strike price and expiration
...
if symbol:
    ...
    model, accuracy, X_test, y_test = train_model(stock_data, symbol)  # Pass symbol for earnings data
