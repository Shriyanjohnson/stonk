...
# Function to fetch stock data including earnings data
@st.cache_data
def fetch_stock_data(symbol):
    ...
    latest_earnings = earnings.iloc[-1]  # Use the most recent earnings data
    earnings_surprise = latest_earnings['Surprise Percent'] if 'Surprise Percent' in latest_earnings else 0
    earnings_date = latest_earnings.name.date()  # Date of earnings report
    
    # Handle NaN values
    data.dropna(inplace=True)
    
    return data, earnings_surprise, earnings_date
...
def generate_recommendation(data, sentiment_score, model, earnings_surprise, earnings_date):
    ...
    # Adjust based on earnings surprise
    if earnings_surprise > 10:  # Large positive surprise
        option = "Call"
    elif earnings_surprise < -10:  # Large negative surprise
        option = "Put"
...
if symbol:
    stock_data, earnings_surprise, earnings_date = fetch_stock_data(symbol)
    ...
    option, strike_price, expiration, latest_data = generate_recommendation(stock_data, sentiment_score, model, earnings_surprise, earnings_date)
    ...
    <div class="recommendation">
        ...
        <p>Latest Earnings Surprise: **{earnings_surprise:.2f}%**</p>
        <p>Last Earnings Report Date: **{earnings_date}**</p>
    </div>
